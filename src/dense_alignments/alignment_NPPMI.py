...
class Concept2Base(object):
    ...
    def sample_max_base(self, one2one=True):
        ...
        for i in range(self.aligner.E.shape[1]):
            col = self.aligner.E[:,i]#((self.aligner.E.getcol(i)).toarray().T)[0, :]
            # sample_size = np.max(20, )
            print("col:", col, len(col))
            ind = np.argpartition(col, -10)[-10:]  # max 20 indices
            ind[np.argsort(col[ind])]
            words = [self.aligner.i2w[j] for j in ind]
            # nonzero = [self.aligner.i2w[ind] for ind, val in sorted(enumerate(col), reverse=True, key=lambda t: float(t[1])) if val > 0][0:20]
...
class Base2Concept(object):
    ...
    def sample_max_concept(self, one2one=True):
        ...
        close_concept_dict = pickle.load(open("../results/close_concepts/dict/"+self.aligner.concept_name+".p", "rb"))
        for i in range(self.aligner.E.shape[1]):
            col = self.aligner.binarize_embedding()[:,i]#((self.aligner.E.getcol(i)).toarray().T)[0, :]
...
class Aligner(object):
    ...
    def binarize_embedding(self):
        binaryE = []
        for i in range(self.E.shape[1]):
            col = self.E[:, i]
            m = np.mean(col)
            new_col = np.where(col>=m, 1.0, 0.0)
            binaryE.append(new_col)
        return np.matrix(binaryE).T
    ...
    def product(self):
        ...
        word_base_binary = self.binarize_embedding()
...
def main():
    ...
    parser.add_argument('--thd', required=False, type=int, default=5,
