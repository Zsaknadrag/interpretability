...
class SparseEmbedding(object):
    ...
    def __init__(self, embedding_path, matrix_name=None, languages=None, filter_rows=-1):
        if matrix_name == None:
            ...
            # matrix_name = "".join( (matrix_name.strip().split("."))[0:-1])
            print(matrix_name)
        ...
        with gzip.open(path, 'rb') as f:
            for line_number, line in enumerate(f):
                ...
                if languages is not None and language not in languages:
                    continue
                if len(parts) > 2001:
                    word = '_'.join(parts[:-2000])
                else:
                    word = parts[0].strip()
                i2w[len(i2w)] = word  # (parts[0].strip().split(":"))[-1] # leave out language
                for i, value in enumerate(parts[-2000:]):
                    value = float(value)
                    if value != 0:
...
def main():
    ...
    parser.add_argument('--embedding', required=True, type=str, help='Path to gzipped contextual embedding.')
    ...
    se = SparseEmbedding(args.embedding, args.matrix_name, args.language)
