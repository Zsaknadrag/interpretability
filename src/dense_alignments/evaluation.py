...
class Evaluator(object):
    def __init__(self, alignment_path, out_path):
    ...
    def load_files(self, alignment_path):
        sparse_path = "../data/dense_matrices/word_base/embeddings/filtered/" + self.random2str() + self.sparse_name + ".p"
        if alignment_path.find("random") != -1:
            sparse_path = "../data/dense_matrices/word_base/" + self.random2str() + self.sparse_name + ".npz"
    ...
    def mp(self, k=50):
        ...
        for i in range(self.E.shape[1]):
            ...
            if len(concepts) > 0:
                ...
                # all = [self.i2w[original_index] for (original_index, value) in col if value > 0]
    ...
    def test_set_evaluation_by_base(self, k=5):
        if self.sparse_name.find("animals") != -1:
            ...
            frequency_name = os.path.join("../data/word_frequency",
                                      ((self.sparse_name.strip().split("_f_"))[0] + "_frequency.p"))
        ...
        # print("Average accuracy for tests by base: ", avg_acc, avg_k_acc)
    ...
    def p_value(self):
        ...
        for i in range(self.E.shape[1]):
            ...
            if len(concepts) > 1 or  ('a' not in concepts and len(concepts) > 0):
                ...
                p_values.append(1.0)
        print("avg p_val: ", sum(p_values)/self.E.shape[1])
...
def main():
    ...
    parser.add_argument('--alignment', required=True, type=str,
    ...
    ev = Evaluator(args.alignment, args.out)
